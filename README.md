...
##### Tamaño de los batch
Como restricción adicional se debe asegurar que los paquetes de *batch* no excedan los 8kB. Para ello se tiene en cuenta que si se toma un cliente con nombre (primero y segundo) cada uno de 10 caracteres cada uno separados por un espacio, así como un solo apellido de 10 caracteres, un documento de 8 dígitos y un número de apuesta de 4 dígitos entonces sabiendo que se usan 4 comas como delimitadores y el formato de la fecha de nacimiento es `YYYY-MM-DD` (10 caracteres), se tiene que solo la información de cada apuesta en el *batch* usaría 57 bytes. Si se toma entonces un único byte para definir un entero sin signo, cada apuesta ocuparía 58 bytes, demostrando que se puede usar un único byte para determinar el tamaño en bytes de cada apuesta. Entonces, como aproximación, si se concatenaran todas las apuestas de este formato, redondeando a 64 bytes por apuesta, entonces se tiene que se pueden enviar exactamente 128 apuestas por *batch*.
