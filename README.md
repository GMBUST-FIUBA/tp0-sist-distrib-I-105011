...
**ADVERTENCIA**: Antes de ejecutar el script es necesario instalar la librería PyYaml (la cual puede no estar en el entorno virtual, si se usa uno). La misma puede instalarse a partir del siguiente comando de terminal:
...
El formato de los paquetes en el protocolo es el siguiente: `<LONGITUD_DE_MENSAJE><TIPO_DE_COMANDO><DATA>`. Primero se tienen dos bytes para almacenar la longitud total del mensaje (es decir los datos y el tipo de comando), luego se usa un byte para definir el tipo de comando y por último vienen los datos del mensaje. Para el comando de agregado de una apuesta a un número se usará el valor de ASCII de la _A_ de tipo de mensaje pero más comúnmente se lo conocerá como comando *ADD*.
...
1) La agencia envía el comando _ADD_ con los siguientes datos en texto `${NOMBRE},${APELLIDO},${DOCUMENTO},${NACIMIENTO},${AGENCIA},${NUMERO}`.
...
3) Como se dijo que la apuesta es totalmente nueva y que el usuario es mayor de edad se almacena la apuesta y se responde con un _OK_, que en el protocolo se indica con un tipo de comando de valor 0.
...
- *ERR NOT_ADULT* (tipo de comando de valor 1): El usuario es menor de edad.
- *ERR NUMBER_TAKEN* (tipo de comando de valor 2): El número ya fue usado en otra apuesta por otra persona.
- *ERR REPEATED_BET* (tipo de comando de valor 3): La persona repitió apuesta (mismo número que una apuesta anterior).
- *ERR NOT_VALID_NUMBER* (tipo de comando de valor 4): El número no es válido (no pertenece al rango de números de la lotería).
- *ERR NOT_VALID_DNI* (tipo de comando de valor 5): El número de DNI no es válido.

Cada uno de los errores está debidamente registrado en los *logs*.

Ahora bien, para evitar problemas en cuanto a los _short read_ y _short write_ se envía, antes de cada mensaje, 2 bytes con la longitud del mensaje codificado en bytes. Cabe aclarar que se utilizará el *encoding* de UTF-8 para los caracteres en este protocolo y que se utilizará *Big Endian* para serializar los mensajes (más que nada la longitud), por ende primero se codifica el mensaje y luego se calcula la longitud en bytes del mismo. Esto se hace para siempre saber lo que se espera por el socket y poder llevar un registro de los caracteres restantes, evitando así los fenómenos mencionados.
...
#### Resolución de ejercicio 6

##### Modificación al protocolo
Se agrega un nuevo comando *ADDB* que permite agregar un *batch* de apuestas al servidor. El tipo de comando es el valor ASCII de la letra _B_. El formato del comando es como sigue: `B<CANTIDAD_DE_APUESTAS><AGENCIA>{APUESTA_1};{APUESTA_2};...;{APUESTA_N}`, donde `APUESTA_N = <LONGITUD_DE_APUESTA>${NOMBRE},${APELLIDO},${DOCUMENTO},${NACIMIENTO},${NUMERO}` para cualquier valor de `N` de la misma forma en que se enviaban las apuestas individuales.

##### Tamaño de los batch
Como restricción adicional se debe asegurar que los paquetes de *batch* no excedan los 8kB. Para ello se tiene en cuenta que si se toma un cliente con nombre (primero y segundo) cada uno de 10 caracteres cada uno separados por un espacio, así como un solo apellido de 10 caracteres, un documento de 8 dígitos y un número de apuesta de 4 dígitos entonces sabiendo que se usan 4 comas como delimitadores, un punto y coma y el formato de la fecha de nacimiento es `YYYY-MM-DD` (10 caracteres), se tiene que solo la información de cada apuesta en el *batch* usaría 58 bytes. Si se toma entonces un único byte para definir un entero sin signo, cada apuesta ocuparía 58 bytes, demostrando que se puede usar un único byte para determinar el tamaño en bytes de cada apuesta. Entonces, como aproximación, si se concatenaran todas las apuestas de este formato, redondeando a 64 bytes por apuesta, entonces se tiene que se pueden enviar exactamente 128 apuestas por *batch*.

Ahora bien, no todas las apuestas van a poseer los datos con el formato que se dijo (sobre todo en los nombres) pero es una buena forma de aproximar la cantidad de apuestas que pueden enviarse por *batch*. Incluso tomando otro apellido de 10 caracteres separados por un espacio se podrían enviar 118 apuestas completas. Por lo tanto se define que la cantidad máxima de apuestas por *batch* será de __100 apuestas__. Teniendo en cuenta el último caso analizado (con dos apellidos largos) se tiene que se utilizarían como máximo 6,7kB para enviar todas las apuestas. No se harán análisis minuciosos del tamaño de los paquetes porque para definir la cantidad máxima de apuestas por _batch_ se busca una opción conservadora para el número de apuestas máximo.

Por último se debe notar que en el análisis no se incluyó en el análisis el encabezado del mensaje `ADDB<CANTIDAD_DE_APUESTAS><AGENCIA>` ya que aún tomando 4 bytes para la cantidad de apuestas del mensaje y el número de agencia, y sumando la cantidad de bytes total del mensaje (2 bytes) se tienen únicamente 14 bytes de encabezado, que es 2 órdenes de magnitud menor que la cantidad de bytes disponibles como máximo con lo cual no afectan al resultado de manera significativa. De todas formas se decide que `<CANTIDAD_DE_APUESTAS>` ocupe un byte ya que como mucho se pueden contar 256 apuestas y como `<AGENCIA>` puede valer solo 5 valores (del 1 al 5) entonces también se toma como un byte.
