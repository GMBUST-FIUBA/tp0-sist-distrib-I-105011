...
Por último se debe notar que en el análisis no se incluyó en el análisis el encabezado del mensaje `B<CANTIDAD_DE_APUESTAS><AGENCIA>` ya que aún tomando 4 bytes para la cantidad de apuestas del mensaje y el número de agencia, y sumando la cantidad de bytes total del mensaje (2 bytes) se tienen únicamente 11 bytes de encabezado, que es 2 órdenes de magnitud menor que la cantidad de bytes disponibles como máximo con lo cual no afectan al resultado de manera significativa. De todas formas se decide que `<CANTIDAD_DE_APUESTAS>` ocupe un byte ya que como mucho se pueden contar 256 apuestas y como `<AGENCIA>` puede valer solo 5 valores (del 1 al 5) entonces también se toma como un byte.
